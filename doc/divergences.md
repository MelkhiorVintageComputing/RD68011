...
Everything here is a divergence that stands *now*. Bugs that were found and
fixed are not divergences any more, and their write-ups have moved to
`doc/bugs-found.md`.
...
way -- a faulted write that did not record its data, and an address error that
fired again on a resumed access. Both are written up in `doc/bugs-found.md`,
along with everything else this project has found and fixed.
